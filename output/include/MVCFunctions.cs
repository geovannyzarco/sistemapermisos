...
//			HttpContext.Current.Response.Write(GlobalVars.Buffer.ToString());
        ...
        public static XVar GetTemplateName(XVar table, XVar templateName)
        {
			...
			if( templateName == "" )
			{
				return ".global/" + table.ToString() + ".cshtml";
	        }
		...
		public static XVar runner_mail(XVar var_params)
		{
			if( CommonFunctions.GetGlobalData( "SMTPSecure", "" ) == "tls" )
				return MVCFunctions.runner_mail_old( var_params );
		...
		public static XVar runner_mail_old(XVar var_params)
		{
            ...
            try
            {
				...
				if( "0" == "1" ||  CommonFunctions.GetGlobalData( "SMTPSecure", "" ) == "tls" )
				{
					smtp.EnableSsl = true;
				}
		...
		{
		// ??
		//			GlobalVars.message = MVCFunctions.Concat("Se ha producido un error", ": ", ex.Message);
		}
...
//			HttpContext.Current.Response.Cookies.Remove(name);
		...
		public static XVar callDashboardSnippet(dynamic _param_name, object _param_eventsObject)
		{
			...
			MethodInfo method = _param_eventsObject.GetType().GetMethods().Where(m => m.Name == "event_" + MVCFunctions.GoodFieldName( _param_name.ToString() ).ToString() ).First();
		...
		public static XVar cloneArray( XVar arr ) {
			return MVCFunctions.my_json_decode( MVCFunctions.my_json_encode(arr) );
		}
